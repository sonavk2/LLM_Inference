...
def parse_args():
    ...
    p.add_argument(
        ...
        help="Override the model's max sequence length. Set this to >= max(context_lengths)+max_new.",
    )
    p.add_argument(
        "--gpu-mem-util", type=float, default=0.9,
        help="vLLM gpu_memory_utilization (KV-cache pool fraction). Default 0.9.",
    )
    p.add_argument(
...
def main():
    ...
    if model_cfg.get("is_vlm"):
        raise SystemExit(
            "Phase 4 vLLM sweep does not support VLMs in this scaffold; pick a text model."
        )
    ...
    for ctx in args.context_lengths:
        ...
        print(f"\n--- ctx={ctx}, batch={args.batch_size}, native={is_native} ---", flush=True)
